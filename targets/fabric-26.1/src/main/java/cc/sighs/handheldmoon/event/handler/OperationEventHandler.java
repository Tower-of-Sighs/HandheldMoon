...
public final class OperationEventHandler {
    ...
    public static void onKey(int key, int action) {
        ...
        if (key == ModKeyBindings.OPEN_DEVICE_CONFIG.getDefaultKey().getValue() && action == InputConstants.PRESS) {
            DeviceConfigOpenHelper.openForCursorTarget();
            return;
        }
        if (key == ModKeyBindings.FLASHLIGHT_SWITCH.getDefaultKey().getValue()) {
    ...
    public static boolean onMouseButton(int button, int action) {
        Player player = Minecraft.getInstance().player;
        if (player == null || !ModKeyBindings.FLASHLIGHT_SWITCH.isDown()) {
            return false;
        }
