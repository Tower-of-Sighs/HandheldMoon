...
public class OperationEventHandler {
    ...
    public static void onKey(KeyEvent event) {
        ...
        if (event.key() == KeyBindings.FLASHLIGHT_SWITCH.key.getValue()) {
            if (event.modifiers() == InputConstants.PRESS) {
                cacheGama = Config.LIGHT_INTENSITY.get();
            }
            if (event.modifiers() == InputConstants.RELEASE) {
    ...
    public static boolean wheelAction(MouseButtonInfo mouseButtonInfo, int action) {
        Player player = Minecraft.getInstance().player;
        if (player == null) return false;
        if (KeyBindings.FLASHLIGHT_SWITCH.isDown() && action == InputConstants.RELEASE) {
