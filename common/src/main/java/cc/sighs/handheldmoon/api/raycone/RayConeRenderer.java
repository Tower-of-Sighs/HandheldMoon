...
public final class RayConeRenderer {
    ...
    public interface Backend {
        void render(PoseStack poseStack, Vec3 cameraPos, Matrix4fc modelViewMatrix, List<ConeSource> sources);
    }

    private static Backend backend = (poseStack, cameraPos, modelViewMatrix, sources) -> { };

    private RayConeRenderer() {
    }

    public static void installBackend(Backend rendererBackend) {
        backend = java.util.Objects.requireNonNull(rendererBackend);
    }
    ...
    public static void render(
        ...
        backend.render(poseStack, cameraPos, modelViewMatrix, sources);
    }
